...
Ihr braucht weder eine Testdatenbank noch eine Keycloak-Instanz für Tests. [Dev Services](https://quarkus.io/guides/dev-services) startet beides automatisch als Container, sobald die zugehörige URL nicht konfiguriert ist, und benutzt dafür intern [Testcontainers](https://java.testcontainers.org/). Voraussetzung ist nur, dass in der `application.properties` keine Datenbank- und keine Keycloak-URL steht. Genau deshalb bleibt sie im Projekt minimal, und die echten Werte kommen in Produktion über [Umgebungsvariablen](./microservices#konfiguration-in-produktion). Was Dev Services sonst noch für euch erledigt, steht auf der Seite [Lokal entwickeln](./lokale-entwicklung#dev-services-datenbank-und-keycloak-ohne-installation).
...
Jeder Testlauf startet damit mit leerem Schema. Tests, die auf Datenresten des vorherigen Laufs basieren, sind die häufigste Ursache für Tests, die mal grün und mal rot sind.
:::
...
1. `./mvnw quarkus:dev` starten. Der Keycloak Dev Service startet automatisch mit, solange in der `application.properties` wie [oben beschrieben](#dev-services-datenbank-und-keycloak-ohne-konfiguration) keine `auth-server-url` gesetzt ist.
